...
// Returns negative value because we minimize distance (lower = better)
// Formula: -(a·b)
...
// PQDistance calculates distance using product quantization codes
// This is an asymmetric distance computation: query is full precision, db vector is quantized
func PQDistance(query []float32, pqCode []byte, codebooks [][][]float32, subvectorDim int) float32 {
	var distance float32
	numSubvectors := len(pqCode)

	for sv := 0; sv < numSubvectors; sv++ {
		code := pqCode[sv]
		centroid := codebooks[sv][code]

		// Calculate distance for this subvector
		startDim := sv * subvectorDim

		for i := 0; i < subvectorDim && startDim+i < len(query); i++ {
			diff := query[startDim+i] - centroid[i]
			distance += diff * diff
		}
	}
